    ...
    if (Sound.music.dom.paused == false) {
        ...
        Sound.music.fadeOutInterval = setInterval(function() {
            if (Sound.music.dom.volume > 0) {
                ...
                Sound.music.dom.volume =
                    Math.round(Sound.music.dom.volume * 100) / 100;
            } else {
    ...
    for (i = 0; i < characters.length; i++) {
        ...
        if (characters[i].Name == "") {
            ...
            playerobject.radio.addEventListener("click", function() {
                ...
                playerobject.allies = Game.traps[this.getAttribute("data-house")].friendlyTo;
        ...
        if(Game.unecessaryInfo == true) {
            if (characters[i].Titles.length == 1) {
                ...
                playerobject.chartitles.innerHTML =
                    ...
                    characters[i].Titles[
                        Math.round(
                            Math.random() * (characters[i].Titles.length - 1)
                        )
                    ] +
            ...
            if (characters[i].Aliases.length == 1) {
                ...
                playerobject.charaliases.innerHTML =
                    ...
                    characters[i].Aliases[
                        Math.round(
                            Math.random() * (characters[i].Aliases.length - 1)
                        )
                    ] +
                    `</span>`; // Get 1 random title
                playerobject.cardinner.appendChild(playerobject.charaliases);
...
Modal.missingChar = function(missing) {
    if (missing == "p1") {
        ...
        Modal.paragraph.innerHTML =
            `Please select a character for: ` +
            `<b data-player='1'> player one </b>`;
        Modal.choiceBtn.textContent = "Go back";
        Modal.choiceBtn.setAttribute(
            "data-modal-close",
            "charSelectionMessage"
        );
        Modal.choiceBtn.addEventListener("click", function(e) {
            Modal.hide("charSelectionMessage");
            Sound.sfx.click();
...
Cards.sliderDrag = function(playerobject, playerid) {
    ...
    function start(e) {
        ...
        playerobject.slider.container.classList.add(
            "characters__slider--active"
        );
        playerobject.slider.container.classList.add(
            playerid + "__slider--active"
        );
        Instructions.drag.classList.remove(
            "characters__draginstruction--hover"
        );
        ...
        if (Player.message.isIt == "1") {
            ...
            if (Game.cpu == true) {
                Player.message.content.innerHTML =
                    `<span data-player='2'>` +
                    P2.name +
                    ` (CPU)</span> goes first`; // Create label
            } else {
                Player.message.content.innerHTML =
...
Game.trapRun = function(player, playerobject, trap) {
    ...
    if (trapAllies == playerAllies && Game.trapsAreAllEnemies == false || Game.trapsAreAllFriendly == true) {
        ...
        if (Game.cpu == true) {
            if (playerobject == P2) {
                Player.message.content.innerHTML="<span data-player='"+playerobject.num+"'>"+playerobject.name+" (CPU) </span>"+trap.friendlyMessage+trap.messageStyle+boost+"</span> tiles.";
            } else {
        ...
        if (trap.penalty == "random") {
            ...
            if (Game.cpu == true) {
                if (playerobject == P2) {
                    Player.message.content.innerHTML="<span data-player='"+playerobject.num+"'>"+playerobject.name+" (CPU) </span>"+trap.message+trap.messageStyle+trap.addition+"</span> tiles.";
                } else {
            ...
            trap.addition = Math.floor(
                Math.random() * Math.floor(trap.penaltyTo)
            );
            if (Game.cpu == true) {
                if (playerobject == P2) {
                    Player.message.content.innerHTML="<span data-player='"+playerobject.num+"'>"+playerobject.name+" (CPU) </span>"+trap.message+trap.messageStyle+trap.addition+"</span> tiles.";
    ...
    setTimeout(function() {
        ...
        setTimeout(function() {
            ...
            playerobject.token.setAttribute("data-tile",playerobject.pos + trap.addition); // Add penalty
            ...
            if (reroll == true) {
                ...
                setTimeout(function() {
                    ...
                    Player.message.content.innerHTML =
                        `<span data-player='1'>` +
                        P1.name +
                        `</span> gets to reroll!`; // Reroll player message
                ...
                setTimeout(function() {
                    ...
                    if (Game.cpu == true) {
                        Player.message.content.innerHTML =
                            `<span data-player='2'>` +
                            P2.name +
                            `'s (CPU)</span> turn`; // Player twos turn message
                    } else {
                        Player.message.content.innerHTML =
                            `<span data-player='2'>` +
                            P2.name +
                            `'s</span> turn`; // Player twos turn message
                    }
...
Difficulty.adjust = function(value) {
    if (value == "childlike") {
        ...
        Game.traps.martell.friendlyBoost = Game.traps.martell.friendlyBoost.slice(0,1);

        Game.traps.dondarrion.penaltyTo *= multiplier;
        Game.traps.dondarrion.friendlyBoost = Game.traps.dondarrion.friendlyBoost.slice(0,1);

        Game.traps.baratheon.penaltyTo *= multiplier;
        Game.traps.baratheon.friendlyBoost = Game.traps.baratheon.friendlyBoost.slice(0,1);

        Game.traps.targaryen.penaltyTo *= multiplier;
        Game.traps.targaryen.friendlyBoost = Game.traps.targaryen.friendlyBoost.slice(0,1);

        Game.traps.mormont.penaltyTo *= multiplier;
        Game.traps.mormont.friendlyBoost = Game.traps.mormont.friendlyBoost.slice(0,1);

        Game.traps.greyjoy.penaltyTo *= multiplier;
        Game.traps.greyjoy.friendlyBoost = Game.traps.greyjoy.friendlyBoost.slice(0,1);

        Game.traps.baelish.penaltyTo *= multiplier;
        Game.traps.baelish.friendlyBoost = Game.traps.baelish.friendlyBoost.slice(0,1);

        Game.traps.bolton.penaltyTo *= multiplier;
        Game.traps.bolton.friendlyBoost = Game.traps.bolton.friendlyBoost.slice(0,1);
...
Game.trapsReload = function() {
    Game.trapsArray = Object.values(Game.traps);
    for (i = 0; i < Game.trapsArray.length; i++) {
        let currentTrapTile = document.getElementById(
            "t" + Game.trapsArray[i].pos
        );
        currentTrapTile.classList.add("trap");
        currentTrapTile.classList.add("trap" + Game.trapsArray[i].modclass);
    }
